...
export default function useLatencyLogger({}) {
    ...
    //function formatFloat (val   ) {return val.toFixed(3).padStart(10, ' ');       };
    ...
    useEffect(() => {
        ...
        logWithColor(`Starting wall-clock time: ${wallStartRef.current} | ${monoStartRef.current}`);
    }, []);
    ...
    const logTime = (prefix, ref) => {
        ...
        logWithColor(`${prefix} ${nowMono}, ${nowWall} | ${toWallClock(nowMono)}`);
        ref.current = nowMono;
    };
    ...
    const logStartEnd = (service, timesRef, startingRef) => {
        ...
        logWithColor(`[${service}] Finished at: ${nowMono}, ${nowWall} | ${toWallClock(nowMono)}`);
